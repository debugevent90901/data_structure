...
class Btree:
    ...
    def __adjust(self, root, single_node, second_list):
        ...
        if root.parent is None:
            ...
            self.__insert_to_root(
                new_parent, single_node.key, single_node.data,
            ...
            parent = root.parent
            self.__insert_to_root(parent, single_node.key, single_node.data,
                                  second_list)
    ...
    def __delete_in_root(self, root, key):
        ...
        if root.numberKeys > self.min:
            ...
            # choose one to use
            if left_sibling_tuple is not None and left_sibling_tuple[
                    0].numberKeys > self.min:
                # choose left
                ...
                self.__delete(
                    left_sibling,
                    left_sibling.last.key)  # left_sibling might not be leaf
                self.__insert_to_root(root, temp_key, temp_data)
            elif right_sibling_tuple is not None and right_sibling_tuple[
                    0].numberKeys > self.min:
                # choose right
                ...
                self.__delete(
                    right_sibling,
                    right_sibling.first.key)  # right_sibling might not be leaf
                self.__insert_to_root(root, temp_key, temp_data)
            else:
                # merge things here
                if left_sibling_tuple is not None:
                    ...
                    self.__insert_to_root(left_sibling, separator.key,
                                          separator.data, root.child)
                    ...
                    self.__insert_to_root(root, separator.key, separator.data,
                                          right_sibling.child)
